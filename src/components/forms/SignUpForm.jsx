...
import { formatCPF, formatWhatsApp } from '@/utils/formatters/maskUtils';
import { validateCPF } from '@/utils/validators/cpfValidation';
import { validateWhatsApp } from '@/utils/validators/phoneValidation';
    ...
    <form onSubmit={handleSubmit} className="w-full grid grid-cols-1 md:grid-cols-2 gap-x-3 gap-y-3">
      ...
      <div className="space-y-0.5 md:col-span-2">
        ...
        <div className="relative group">
          ...
          <input
            type="text" name="name" placeholder="Seu nome completo" value={formData.name} onChange={handleChange} onBlur={handleBlur}
            className={`w-full pl-10 pr-10 py-2.5 text-sm border rounded-lg bg-slate-50 focus:bg-white transition-all outline-none focus:ring-2 ${getFieldStatus('name') === 'error' ? 'border-red-300 focus:ring-red-200' : getFieldStatus('name') === 'success' ? 'border-emerald-200 focus:ring-emerald-200' : 'border-slate-200 focus:ring-emerald-200'}`} required
          />
        </div>
      ...
      <div className="space-y-0.5">
        ...
        <div className="relative group">
          ...
          <input
            type="email" name="email" placeholder="seu@email.com" value={formData.email} onChange={handleChange} onBlur={handleBlur}
            className={`w-full pl-10 pr-10 py-2.5 text-sm border rounded-lg bg-slate-50 focus:bg-white transition-all outline-none focus:ring-2 ${getFieldStatus('email') === 'error' ? 'border-red-300 focus:ring-red-200' : getFieldStatus('email') === 'success' ? 'border-emerald-200 focus:ring-emerald-200' : 'border-slate-200 focus:ring-emerald-200'}`} required
          />
        </div>
      ...
      <div className="space-y-0.5">
        ...
        <div className="relative group">
          ...
          <input
            type="tel" name="phone" placeholder="(00) 00000-0000" value={formData.phone} onChange={handleChange} onBlur={handleBlur} maxLength={15}
            className={`w-full pl-10 pr-10 py-2.5 text-sm border rounded-lg bg-slate-50 focus:bg-white transition-all outline-none focus:ring-2 ${getFieldStatus('phone') === 'error' ? 'border-red-300 focus:ring-red-200' : getFieldStatus('phone') === 'success' ? 'border-emerald-200 focus:ring-emerald-200' : 'border-slate-200 focus:ring-emerald-200'}`} required
          />
        </div>
      ...
      <div className="space-y-0.5">
        ...
        <div className="relative group">
          ...
          <input
            type="text" name="cpf" placeholder="000.000.000-00" value={formData.cpf} onChange={handleChange} onBlur={handleBlur} maxLength={14}
            className={`w-full pl-10 pr-10 py-2.5 text-sm border rounded-lg bg-slate-50 focus:bg-white transition-all outline-none focus:ring-2 ${getFieldStatus('cpf') === 'error' ? 'border-red-300 focus:ring-red-200' : getFieldStatus('cpf') === 'success' ? 'border-emerald-200 focus:ring-emerald-200' : 'border-slate-200 focus:ring-emerald-200'}`} required
          />
        </div>
      ...
      <div className="space-y-0.5">
        ...
        <div className="relative group">
          ...
          <input
            type={showPassword ? "text" : "password"} name="password" placeholder="Mínimo 6 caracteres" value={formData.password} onChange={handleChange} onBlur={handleBlur}
            className={`w-full pl-10 pr-12 py-2.5 text-sm border rounded-lg bg-slate-50 focus:bg-white transition-all outline-none focus:ring-2 ${getFieldStatus('password') === 'error' ? 'border-red-300 focus:ring-red-200' : getFieldStatus('password') === 'success' ? 'border-emerald-200 focus:ring-emerald-200' : 'border-slate-200 focus:ring-emerald-200'}`} required
          />
          <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors p-1">
      ...
      <div className="space-y-0.5">
        ...
        <div className="relative group">
          ...
          <input
            type={showConfirmPassword ? "text" : "password"} name="confirmPassword" placeholder="Repita sua senha" value={formData.confirmPassword} onChange={handleChange} onBlur={handleBlur}
            className={`w-full pl-10 pr-12 py-2.5 text-sm border rounded-lg bg-slate-50 focus:bg-white transition-all outline-none focus:ring-2 ${getFieldStatus('confirmPassword') === 'error' ? 'border-red-300 focus:ring-red-200' : getFieldStatus('confirmPassword') === 'success' ? 'border-emerald-200 focus:ring-emerald-200' : 'border-slate-200 focus:ring-emerald-200'}`} required
          />
          <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-slate-600 transition-colors p-1">
      ...
      <div className="pt-2 md:col-span-2">
        ...
        >
          {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" />Criando conta...</> : 'Criar conta gratuita'}
        </motion.button>
      </div>
    </form>
  );
